...
// Import komponen dasar shadcn dialog untuk konsistensi UI
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function AdminSidebar() {
  ...
  return (
    <>
      <Sidebar className="border-r border-[#5a8a2d]/20 bg-white text-slate-800">
        ...
        <SidebarFooter className="p-4 border-t border-[#5a8a2d]/20">
          <button
            type="button"
            onClick={() => {
              setOpenMobile(false); // Menutup otomatis laci sidebar mobile di layar HP
              setShowLogoutModal(true);
            }}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-600 hover:bg-red-50 hover:text-red-500 transition-all group cursor-pointer"
          >
      ...
      {/* MODAL SHADCN DIALOG LOGOUT (STRUKTUR IDENTIK SINKRON) */}
      <Dialog
        ...
        <DialogContent className="sm:max-w-xs rounded-xl p-5">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs text-zinc-400 mt-1">
              Anda akan keluar dari sesi ini. Pastikan semua pekerjaan admin sudah tersimpan sebelum melanjutkan.
            </DialogDescription>
          </DialogHeader>
